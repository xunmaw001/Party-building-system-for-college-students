                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="exampaperName" id="exampaperName" placeholder="试卷名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="exampaperTypes" id="exampaperTypes" lay-filter="exampaperTypes">
                                  ...
                                  <option v-for="item in exampaperTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                    ...
                    <section style="margin: 10px">
                        <div class="row">
                            <div v-for="(item,index) in dataList" class="col-md-3">
                                <div class="gallery-demo" @click="jumpCheck('../exampaper/detail.jsp?id='+item.id , item.aaaaaaaaaa == null?'':item.aaaaaaaaaa , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                    <img :src="item.exampaperPhoto?item.exampaperPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                    <h4 class="p-mask">{{item.exampaperName}}
                                        <span v-if="item.exampaperNewMoney"> - ￥{{item.exampaperNewMoney}}</span>
                                        <span v-else> - {{item.exampaperValue}}</span>
                                    </h4>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    // 当前表的字典表
        ...
        function exampaperTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=exampaper_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function pageList() {
            ...
            if (jquery('#exampaperName').val() != null && jquery('#exampaperName').val() != "" && jquery('#exampaperName').val() != "null" ) {
                param['exampaperName'] = jquery('#exampaperName').val();
            }
                         
            var exampaperTypes = $("#exampaperTypes").find("option:selected").val();
            if (exampaperTypes != null && exampaperTypes != "" && exampaperTypes != "null") {
                param['exampaperTypes'] = exampaperTypes;
            }
                

            // 获取列表数据
            http.request('exampaper/list', 'get', param, function(res) {
